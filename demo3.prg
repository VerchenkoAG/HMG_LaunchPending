 ...
 *
 * Запуск второй копии программы с ожиданием, команда: SET MULTIPLE OFF WARNING
 * Starting a second copy of the program with the wait, command: SET MULTIPLE OFF WARNING
 */

#define _HMG_OUTLOG
#define SHOW_TITLE  "My test program (3)"
...
PROCEDURE MAIN(cParam)
   ...
   SET MSGALERT BACKCOLOR TO { 178, 227, 137 }               // for HMG_Alert()
   DEFINE FONT DlgFont FONTNAME "DejaVu Sans Mono" SIZE 14   // for HMG_Alert()
   SET FONT TO "DejaVu Sans Mono", 12
   SET OOP ON

   IF cParam == "RESTART"
      cMsg := SHOW_TITLE +": Запуск второй копии программы / Starting a second copy of the program"
      ? cMsg
      SET WINDOW MAIN OFF
      WaitWindow( cMsg, .T. )
      nY := 0
      DO WHILE myIsProgaInMemory(SHOW_TITLE, "HMG_FORM_", .T. )
         ? "." ; ? "-------- Is there a program in memory ["+SHOW_TITLE+",HMG_FORM_] ? ------ For:", ++nY
         wApi_Sleep(500)
      ENDDO
   ...
   ELSE
      ...
      cMsg := "Попытка запуска второй копии программы !;"
      cMsg += App.ExeName + ";Отказано в запуске.;"
      cMsg += _HMG_MESSAGE[4] + ";;"
      cMsg += "Trying to start a second copy of the program!;"
      cMsg += App.ExeName + ";Launch denied.;"
      cMsg += "The program is already running.;"
   ...
   DEFINE WINDOW test                           ;
      AT 310, nX WIDTH nW HEIGHT nH             ;
      TITLE SHOW_TITLE                          ;
      ICON "1MAIN_ICO"                          ;
      MAIN NOMAXIMIZE NOSIZE                    ;
      BACKCOLOR { 178, 227, 137 }               ;
      ...
      DEFINE MAIN MENU
         POPUP "Run"
            ITEM "Запуск копии программы"         ACTION myRestart()
...
FUNCTION MsgAbout()
   ...
   cMsg := SHOW_TITLE + ";;"
   cMsg += "Запуск второй копии программы с ожиданием,; .... команда: SET MULTIPLE OFF WARNING;;"
   cMsg += "Starting a second copy of the program with the wait,; .... command: SET MULTIPLE OFF WARNING;;"
