...
PROCEDURE MAIN(cParam)
   ...
   IF cParam == "RESTART"
      cMsg := 'Запуск второй копии программы / Starting a second copy of the program'
      ? cMsg
      SET WINDOW MAIN OFF
      WaitWindow( cMsg, .T. )
      nY := 0
      // Проверка на окончание первой копии программы по мютексу
      WHILE IsExe2Run() ; wApi_Sleep(500)
         ? "------["+SHOW_TITLE+"]--- Проверка, Мьютекс есть / Check Mutex is ---- For:", ++nY
      END
...
FUNCTION MsgAbout()
   ...
   cMsg := SHOW_TITLE + ";;"
   cMsg += "Запуск второй копии программы с ожиданием, используем мьютекс;"
